...
import numpy as np
plt.style.use('bmh')
def generate_plot_memory(csv_files,saveto):
    ...
    #least squares fit
    total_mem = np.polyfit(x_values, total_memory, 2)
    x_range = np.arange(0, 100, 0.1)
    plt.plot(x_range, np.polyval(total_mem, x_range), label='total')

    train_mem = np.polyfit(x_values, memroy_train, 2)
    x_range = np.arange(0, 100, 0.1)
    plt.plot(x_range, np.polyval(train_mem, x_range), label='train')

    train_evl = np.polyfit(x_values, memroy_eval, 2)
    x_range = np.arange(0, 100, 0.1)
    plt.plot(x_range, np.polyval(train_evl, x_range), label='eval')



    #plot
    plt.plot(x_values, total_memory, marker='o', linestyle='None',ms=5)
    plt.plot(x_values, memroy_train, marker='o', linestyle='None',ms=5)
    plt.plot(x_values, memroy_eval, marker='o', linestyle='None',ms=5)
...
csv_files_action= ['../../checkpoints/gpu/action_lightning/mamba2_5_gpu_stats.csv',
              '../../checkpoints/gpu/action_lightning/mamba2_10_gpu_stats.csv', 
              '../../checkpoints/gpu/action_lightning/mamba2_20_gpu_stats.csv', 
              '../../checkpoints/gpu/action_lightning/mamba2_40_gpu_stats.csv', 
              '../../checkpoints/gpu/action_lightning/mamba2_60_gpu_stats.csv',
              '../../checkpoints/gpu/action_lightning/mamba2_80_gpu_stats.csv',
              '../../checkpoints/gpu/action_lightning/mamba2_100_gpu_stats.csv',]
...
generate_plot_memory(csv_file_importance_sampling,'../figs/scale/lightning/memory_usage_importance_sampling_standard.pdf')


generate_plot_memory_train(csv_files_action,'../figs/scale/lightning/memory_usage_train_action.pdf')
